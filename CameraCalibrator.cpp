...
CameraCalibrator::CameraCalibrator() : flag(0), mustInitUndistort(false) {}

CameraCalibrator::CameraCalibrator(int flag, const bool mustInitUndistort)
{
    this->flag = flag;
...
int CameraCalibrator::addChessboardPoints(const std::vector<std::string> &file_list, CameraCalibrator::cv::Size &board_size)
{
    ...
    for (int i = 0; i < file_list.size(); i++)
    {
        ...
        bool found = cv::findChessboardCorners(img, board_size, imagePoints);
        // get subpixel accuracy on the corners
        // _, _, max number of iterations, min accuracy
        cv::cornerSubPix(img, imagePoints, cv::Size(5, 5), cv::Size(-1, -1), cv::TermCriteria(cv::TermCriteria::MAX_ITER + cv::TermCriteria::EPS, 30, 0.1));
        // if we have a good board add to our data
        if (imagePoints.size() == board_size.area())
        {
            // add image and scene points from one view
            addPoints(imagePoints, object_corners);
            successes++;
        }
        // draw the corners
...
double CameraCalibrator::calibrate(cv::Size &img_size)
{
    ...
    return calibrateCamera(objectPoints, img_points, img_size, camera_matrix, dist_coeffs, rvecs, tvecs, flag);
}
